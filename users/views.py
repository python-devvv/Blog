...
def profiles(request, username):
    ...
    author_top_posts = posts.annotate(
...
def top_authors(request):
    ...
    profiles = Profile.objects.all().annotate(
        num_follow=Count('follow')).order_by('-num_follow')[:5]
