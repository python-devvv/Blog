...
def new_post(request):
    ...
    return render(request, 'blog/new_post.html',
                  {'form': form, 'title': 'Blog - Add new post'})
    ...
    if post.author == request.user:
        ...
        if request.method == 'POST':
            ...
            if edit_form.is_valid():
                edit_form.save()
                messages.success(
                    request, f'Your changes has been updated successfully!!')
...
def bookmark_post(request, slug):
    ...
    if post.bookmark.filter(id=request.user.id).exists():
        ...
        messages.success(
            request,
            f'This post is successfully removed from your Bookmarks!!')
    else:
        post.bookmark.add(request.user)
        messages.success(
            request,
            f'This post is successfully added to your Bookmarks!!')
    return HttpResponseRedirect(post.get_absolute_url())
...
def top_posts(request):
    posts = Post.objects.annotate(
        num_likes=Count('like')).order_by('-num_likes')[:5]
...
def search(request):
    ...
    if search_query:
        results = Post.objects.filter(Q(title__icontains=search_query) | Q(
            content__icontains=search_query)).order_by('-date_posted')
    else:
